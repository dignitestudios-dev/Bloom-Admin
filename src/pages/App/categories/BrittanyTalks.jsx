import BrittanyTalksPostCard from "../../../components/categories/BrittanyTalksPostCard";
...
const BrittanyTalks = ({ id }) => {
  ...
  const getAllposts = () => {
    ...
    axios
      .get(`${baseUrl}/api/allBrittanyTalk/${id}`, { headers })
      .then((response) => {
    ...
    <div className="w-full flex flex-col justify-start items-start gap-4">
      ...
      <div className="w-full h-full grid grid-cols-2 gap-4 justify-start items-start">
        {posts?.map((post, index) => {
          return <BrittanyTalksPostCard post={post} key={index} />;
        })}
      </div>
    </div>
  );
